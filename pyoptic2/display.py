...
def plot2d(rays, sli='xz'):
    ...
    def plot_ray(ray):
        ...
        if sli == 'xz':
            ...
            plt.plot(ra[:, 0, :].squeeze().T, ra[:, 1, :].squeeze().T, c=ray[0].color)
        else:
...
def plot_system2d(sys, sli='xz'):
    if sli == 'xz':
        def plot_surf(s):
            sf = s.surface(proj='y')
            plt.plot(sf[2], sf[0], getattr(s, 'ls', 'k'))
    
    elif sli == 'zx':
        def plot_surf(s):
            sf = s.surface(proj='y')
            plt.plot(sf[0], sf[2], getattr(s, 'ls', 'k'))
    
    elif sli == 'xy':
        def plot_surf(s):
            sf = s.surface(proj='z')
            #print('sf.shape:', sf.shape)
            plt.plot(sf[0], sf[1], getattr(s, 'ls', 'k'))
            
    elif sli == 'yx':
        def plot_surf(s):
            sf = s.surface(proj='z')
            plt.plot(sf[1], sf[0], getattr(s, 'ls', 'k'))
